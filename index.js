...
class Stats {
    ...
    print(){
        ...
        const opts = {
            height: 6,
            format:  function (x, i) { return ((x/1000).toFixed (2)) }
        }
        console.log(asciichart.plot (this.intervals.map(x => x.duration),  opts))
    }
}
...
function doesCollide(circles, centerX, centerY){
    for(const circle of circles){
        if(!(centerX < circle.centerX - DIST * 2 || centerX > circle.centerX + DIST * 2) && 
           !(centerY < circle.centerY - DIST * 2 || centerY > circle.centerY + DIST * 2)){
            return true
        }
    }
    return false
...
function findFreeSpot(circles){
    ...
    while(true){
        ...
        if(centerX - dist < 0 || centerX + DIST > SIZE_X || centerY - DIST < 0 || centerY + dist > SIZE_Y){
            continue
        }
...
function main(gameType) {
    ...
    canvas.addEventListener("click", (e) => {
        stats.click()
        const clickedCircle = circles.find(circle => circle.isInPath(ctx, e.offsetX, e.offsetY))
        if(!clickedCircle){
            stats.misclicks += 1
            return
        }
